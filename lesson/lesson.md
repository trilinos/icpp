...
Go to the directory for the example application
```
cd /workspace/trilinos/build/packages/muelu/example/basic/
```
...
##### Question
Do you see any significant changes in convergence behavior?

No, neither solver manages to converge in less than 100 iterations.

##### Question
Why is it preferable to use conjugate gradients over GMRES in this case?
...
```
export KOKKOS_TOOLS_LIBS=/workspace/kokkos-tools/build/profiling/memory-usage/libkp_memory_usage.so
...
##### Question
Based on the iterations you recorded, how does this condition number roughly scale with respect to the number of unknowns?
...
##### Question
How does the operator complexity help the user assess potential performance?
...
##### Question
Why might the operator complexity and smoother complexity differ?'
...
The Gauss-Seidel smoother convergence degrades slightly as the number of MPI ranks is increased.  The Chebyshev smoother convergence is unaffected by the number of ranks.-->

<!--##### Question
Can you explain your observations?

First, when Gauss-Seidel is run with
...
##### Question
Do you see an improvement in iterations?

Yes. For symmetric Gauss-Seidel, the number of iterations is lower and
...
##### Question
Do you think that Gauss-Seidel is easily adaptable for use on massively parallel architectures such as GPUs?
...
##### Question
How does this compare to the smoothers without coloring? How does this compare to a Jacobi smoother?

It performs faster than the
...
<!-- ##### Question
What did you observe in the previous two runs?
...
<!-- ##### Question
What effect does modifying the threshold value have on the multigrid convergence?
...
<!-- ##### Question
Looking at the respective multigrid summaries, can you observe a reduction of non-zeros (nnz)?
